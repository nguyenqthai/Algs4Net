 ...
 *  Runs in O(E + V) time.
 *
 *  C:\> algscmd Bipartite 10 12 20 3
 * 
 ******************************************************************************/
...
{
  /// <summary><para>
  /// The <c>Bipartite</c> class represents a data type for
  ...
  /// <a href="http://algs4.cs.princeton.edu/code/edu/princeton/cs/algs4/Bipartite.java.html">Bipartite</a>
  ...
  public class Bipartite
  {
    ...
    private LinkedStack<int> cycle;  // odd-length cycle
    ...
    /// <returns>the side of the bipartition that vertex <c>v</c> is on; two vertices</returns>
    /// are in the same side of the bipartition if and only if they have the
    ///  same color
    /// <exception cref="IndexOutOfRangeException">unless <c>0 &lt;= v &lt; V</c> </exception>
    /// <exception cref="InvalidOperationException">if this method is called when the graph</exception>
    ///        is not bipartite
    ///
    ...
    /// <returns>an odd-length cycle if the graph is not bipartite (and 
    /// hence has an odd-length cycle), and <c>null</c> otherwise</returns>
    ///
    ...
    private bool check(Graph G)
    {
      ...
      if (isBipartite)
      {
        for (int v = 0; v < G.V; v++)
        {
          foreach (int w in G.Adj(v))
          {
            if (color[v] == color[w])
            {
              Console.Error.WriteLine("edge {0}-{1} with {2} and {3} in same side of bipartition\n", v, w, v, w);
              return false;
            }
          }
        }
      ...
      else
      {
        ...
        if (first != last)
        {
          Console.Error.WriteLine("cycle begins with {0} and ends with {1}\n", first, last);
          return false;
        }
      }
    ...
    /// <summary>
    /// Demo test the <c>Bipartite</c> data type.</summary>
    /// <param name="args">Place holder for user arguments</param>
    /// 
    [HelpText("algscmd Bipartite V1 V2 E F", "V1, V2-vertices on either side, E-number of edges, F-number of random edges")]
