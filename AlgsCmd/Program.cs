...
// The port to .NET Core 6 will use the older Program class structure
using Algs4Net;
using System;
...
namespace AlgsCmd
{
    class Program
    {
        ...
        private static string FlattenedExceptionMessage(Exception ex)
        {
        ...
        private static MethodInfo GetMainTest(string className)
        {
            ...
            for (int i = 0; i < suffixes.Length; i++)
            {
                ...
                if (t1 is not null)
                {
                    ...
                    var method = t1.GetMethod(mainTest);
                    testMethod = method is null ? null : method;
                    break;
                }
            }
            return testMethod;
        ...
        private static void DisplayHelpFor(string className)
        {
            ...
            {
                Console.WriteLine("Error: {0}", Program.FlattenedExceptionMessage(ex));
            }
        }
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Test run error: {0}", Program.FlattenedExceptionMessage(ex));
